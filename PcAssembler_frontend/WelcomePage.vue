<template>
  <div class="welcome-container" :class="{ show: isTextVisible }">
    ...
    <div class="button-group" :class="{ show: areButtonsVisible }">
      <button @click="login" :style="{ transform: areButtonsVisible ? 'translateX(0)' : 'translateX(-100%)' }">Login</button>
      <button @click="goToSignUp" :style="{ transform: areButtonsVisible ? 'translateX(0)' : 'translateX(100%)' }">Sign Up</button>
    </div>
    <button class="guest-button" @click="enterAsGuest" :class="{ show: isGuestButtonVisible }">Enter as Guest</button>
  </div>
</template>
...
.button-group button {
  ...
  border-radius: 4px;
  font-size: 1em;
  transition: background-color 0.3s, transform 0.5s ease-in-out; /* Button hover and slide animation */
}
